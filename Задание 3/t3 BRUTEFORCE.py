...
def faster_example_path_constructor():
    path = [i for i in range(NUMBER_OF_CITIES)]
    path.append(path[0])
    return path.copy()

# строит всевозможные пути для пункта А под индексом 0
def faster_path_constructor():
    example_paths = faster_example_path_constructor()
    all_paths = []
...
FIXED_MATRIX = [[0, 12, 12, 10, 12, 13, 15, 15, 7, 14],
...
FIXED_NUMBER_OF_CITIES = 10 # количество городов для FIXED_MATRIX. ОБЯЗАТЕЛЬНО менять, если у вас не 10 городов.

print("\nВНИМАНИЕ!\nВремя выполнения программы сильно увеличивается при 8 городах и выше!\n\nГраф не обязательно должен быть полным, но обязан удовлетворять задаче коммивояжера.\nВ противном случае ответ будет неправильным.\n")
...
"2) Генерировать случайную матрицу заданного размера с расстоянием от 5 до 15.\n" \
"Выбор по умолчанию: фиксированная матрица 10х10\n"))

match choice:
    case 1:
        PATH_MATRIX =  FIXED_MATRIX
        NUMBER_OF_CITIES = FIXED_NUMBER_OF_CITIES
    case 2:
